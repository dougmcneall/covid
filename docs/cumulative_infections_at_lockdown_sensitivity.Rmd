...
This is a preliminary sensitivity analysis to test data, emulators and techniques, and shouldn't be used to draw conclusions. There appears to be at least one ensemble member where MetaWards "breaks", for example infecting more people than there are in the population.
...
Fit the emulator. Note, there is NO NUGGET and Danny's analysis suggests a considerable variance between repeats for various local authorities.
```{r}
