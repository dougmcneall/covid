...
This vignette examines some estimates of UK cases of COVID-19 through time. These data should prove useful in the history matching of MetaWards. We look at daily lab-confirmed cases and estimates of new cases from the ONS survey. We aggregate the daily new cases to weekly, and interpolate in time to match the ONS survey.
...
plot(week_ax, weekly_new_cases$weekly_sum,
     ylim = c(0, 120000), xlim = as.Date(c("2020-03-01", "2020-06-14")),
     xlab = '', ylab = '',
     pch = 19,
     bty = 'n',
     type = 'n')
...
points(ons_midpoints, ons[,'new_cases_med'], col = 'red', pch = 19)
points(ons_midpoints, pred$mean, col = 'skyblue2', pch = 19)
segments(x0 = ons_midpoints, y0 = ons[,'new_cases_lower95'], x1 = ons_midpoints, y1 = ons[,'new_cases_upper95'], col = 'tomato2')
mtext('Weekly new cases in England', side = 3, adj = 0.06, line = -2,outer = TRUE, cex = 1.5)
legend('topleft', pch = 19, col = c('black', 'skyblue2','red'),
       legend = c('lab-confirmed cases', 'interpolated', 'ONS estimates + 95% CI'),
       bty = 'n')

```


Now we can get an estimate of the proportion of new cases that are picked up daily (and aggregated to weekly), compared to the ONS survey results. It looks like the daily lab-confirmed cases represent between 11% and 12.5% of all cases BUT THERE IS A VERY LARGE UNCERTAINTY ON THE ONS FIGURES which we don't take into account.
```{r}
